...
public static class Sublist
{
        ...
        where T : IComparable
    {
        ...
        else if (list1.Count() > list2.Count())
        {
            ...
            var leftCursor = 0;
            var subRangeLength = list2.Count();
            var rightCursor = leftCursor + subRangeLength;

            while (rightCursor <= list1.Count())
            {
                var subRange = list1.GetRange(leftCursor, subRangeLength);

                if (CompareEqualSizedLists(subRange, list2) == SublistType.Equal)
                {
                    return SublistType.Superlist;
                }
